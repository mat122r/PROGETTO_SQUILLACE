...
def run_check():
    ...
    # 3. Connessione MySQL (localhost:3306, root, senza password)
    try:
        ...
        conn.close()
        print("  [OK] MySQL – connessione a localhost:3306 (root) riuscita")
    except Exception as exc:
        print(f"  [KO] MySQL non raggiungibile: {exc}")
        print("       Soluzione: avviare XAMPP/MySQL, verificare che sia in ascolto su porta 3306.")
        errori += 1
